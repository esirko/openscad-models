...
//Distance of spiral plunger = 152 - 15 = 137
//l-2.5 - 12.5 = 

//up(lnl+fph+5) cuboid([200, 100, 2], anchor=BOTTOM);
//up(12.5) cuboid([200, 100, 2], anchor=TOP);

right(100)
union() {
    up((lnl+fph-slb)/2) diff() generic_threaded_rod(d=major_diameter, l=lnl+fph+slb, pitch=pitch, starts=3, profile=profile, $fn=fn) {
...
difference() {
    union() {
        // main gutter
        diff() cyl(r=gp+st, h=sl+2, $fn=fn, anchor=BOTTOM)
            tag("remove") cyl(r=gp, h=sl+3, $fn=fn)
            tag("remove") cuboid([2*(gp+st+1), gp+st+1, sl+3], anchor=BACK);
        ...
        down(sph-spz) rounded_prism(hexagon(knob_scale), height=knob_height, joint_top=2, joint_bot=2, joint_sides=2, anchor=BOTTOM);
    ...
    union() {
        ...
        tube(h=100, ir=spr, or=40, $fn=fn, anchor=BOTTOM);
    }
}
...
// attachments to experiment with for left-hand grip
slop=0.2;
left(100)
union() {
    difference() {
